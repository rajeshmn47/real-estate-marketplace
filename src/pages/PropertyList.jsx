...
// ---- FilterContent extracted OUTSIDE ----
const FilterContent = ({
    filters,
    setFilters,
    localitySearch,
    ...
    <div className="space-y-4">
        {/* Locality – exactly like homepage */}
        <div>
            <label className="block text-sm text-gray-700 font-medium">Locality</label>
            <div className="relative">
                <input
                    ...
                    onKeyPress={handleLocalityKeyPress}
                />
                {(localitySuggestions.length > 0 || localityLoading) && (
                    <div className="absolute left-0 right-0 top-full z-50 mt-1 bg-white rounded-xl shadow-lg border border-slate-200 overflow-hidden">
                        {localityLoading ? (
                            <div className="p-3 text-center text-sm text-slate-500">Loading...</div>
                        ) : (
                            localitySuggestions.map((locality, idx) => (
                                <button
                                    key={idx}
                                    type="button"
            ...
            {filters.locality && (
                <div className="mt-1 text-xs text-blue-600 flex items-center gap-2">
                    <span>Selected: {filters.locality}</span>
                    <button
        ...
        {/* Project */}
        <div>
            <label className="block text-sm text-gray-700 font-medium">Project</label>
            <div className="relative">
                <input
                    type="text"
                    id="project-search"
                    ...
                    onFocus={() => {
                        if (projectSuggestions.length > 0) setShowProjectDropdown(true);
                    }}
                />
                {showProjectDropdown && projectSuggestions.length > 0 && (
                    <div className="absolute left-0 right-0 top-full z-50 mt-1 bg-white border rounded-lg shadow-lg max-h-48 overflow-y-auto">
            ...
            {selectedProject && (
                <div className="mt-1 text-xs text-blue-600 flex items-center gap-2">
                    ...
                    <button className="text-red-500 hover:text-red-700" onClick={clearProject}>
                        Clear
                    </button>
                </div>
            )}
...
function PropertyList() {
    ...
    const localityDebounce = useRef(null);
    ...
    const fetchFavorites = async () => {
        if (!token) return;
        try {
            const { data } = await api.get(`${API_BASE}/users/me/favorites`);
    ...
    const loadProperties = async () => {
        ...
        try {
            ...
            const locality = q.get('locality') || filters.locality;
            ...
            const project = q.get('project') || filters.project;
    ...
    useEffect(() => {
        ...
        projectDebounce.current = setTimeout(async () => {
            try {
                const res = await axios.get(`${API_BASE}/projects?search=${encodeURIComponent(projectSearch)}`);
                const projects = res.data?.projects || [];
                setProjectSuggestions(projects);
                setShowProjectDropdown(projects.length > 0);
            } catch (err) {
        ...
        return () => clearTimeout(projectDebounce.current);
    }, [projectSearch]);
    ...
    const selectProject = (project) => {
        ...
        setShowProjectDropdown(false);
        setFilters(prev => ({ ...prev, project: project.uuid }));
    };
    ...
    const fetchLocalitySuggestions = async (query) => {
        ...
        try {
            const response = await axios.get(`${API_BASE}/localities?search=${encodeURIComponent(query.trim())}`);
            const suggestions = response.data || [];
            setLocalitySuggestions(suggestions);
            setShowLocalityDropdown(suggestions.length > 0);
        } catch (err) {
            console.error('Locality autocomplete failed', err);
            setLocalitySuggestions([]);
            setShowLocalityDropdown(false);
        } finally {
    ...
    useEffect(() => {
        ...
        localityDebounce.current = setTimeout(() => {
            fetchLocalitySuggestions(localitySearch);
        }, 300);
        return () => clearTimeout(localityDebounce.current);
    }, [localitySearch]);
    ...
    const handleLocalityKeyPress = (e) => {
        if (e.key === 'Enter') {
            if (localitySuggestions.length > 0) {
                selectLocality(localitySuggestions[0]);
            } else {
                setFilters(prev => ({ ...prev, locality: localitySearch }));
                setLocalitySuggestions([]);
                setShowLocalityDropdown(false);
            }
        }
    ...
    // ---- Click outside to close locality dropdown ----
    useEffect(() => {
        const handleClickOutside = (e) => {
            if (localityInputRef.current && !localityInputRef.current.contains(e.target)) {
        ...
        <div className="min-h-screen bg-slate-50">
            ...
            <nav className="sticky top-0 z-50 bg-blue-900 shadow-lg px-3 md:px-5 py-2 md:py-3 flex items-center justify-between gap-2 flex-nowrap whitespace-nowrap">
                <div className="flex items-center gap-2 md:gap-3 flex-shrink-0">
                    ...
                    <div className="hidden md:flex items-center bg-blue-800/60 px-2 md:px-3 py-1 md:py-1.5 rounded-md cursor-pointer hover:bg-blue-700/60 transition border border-blue-400/30 text-xs md:text-sm">
                        ...
                        <span className="font-medium text-white">{filters.locality || 'Select Locality'}</span>
            ...
            <main className="max-w-7xl mx-auto px-4 py-6">
                <div className="grid gap-6 lg:grid-cols-[280px_1fr]">
                    ...
                    <aside className="hidden lg:block rounded-2xl bg-white p-4 sm:p-6 shadow-sm lg:sticky lg:top-24 overflow-visible">
                        ...
                        <FilterContent
                            ...
                            handleLocalityKeyPress={handleLocalityKeyPress}
            ...
            {showFilterModal && (
                ...
                >
                    <div
                        className="bg-white rounded-2xl p-6 max-w-md w-full max-h-[90vh] overflow-visible relative"
